...
```{r echo=FALSE, fig.cap="Ozone Records in 2018"}
epa_data %>%
  filter(Year==2018) %>%
  drop_na(Ozone) %>% 
  ggplot(
  ...
  geom_boxplot()+
  labs(y="Concentration $\\mu g/m^3$", x="Month")+
  ggtitle("Ozone Records in 2018")
  
```

```{r echo=FALSE,fig.cap="Ozone Records in 2019"}
epa_data %>%
  filter(Year==2019) %>%
  ggplot(
    aes(
