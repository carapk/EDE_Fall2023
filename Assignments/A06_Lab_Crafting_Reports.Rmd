...
```{r project.setup, message=FALSE, warning=FALSE, echo=TRUE, results=FALSE}
# message and warning = FALSE to now show messages and errors 
# echo = true to show code, results = false to not show output 
...
# Echo = false to show output only but code is run and not displayed
...
```{r echo=FALSE, fig.align='left', fig.cap="Ozone Records in 2018", fig.height=3.5, message=FALSE, warning=FALSE}
epa_data %>%
  filter(Year==2018) %>%
  drop_na(Ozone) %>% 
  ggplot(
  ...
  geom_boxplot()+
  labs(y=expression(paste("Concentration ", mu, g/m^3)), x="Month")+
  ggtitle("Recorded Ozone by Month (2018)")
  
```

```{r echo=FALSE, fig.align='left', fig.cap="Ozone Records in 2019", fig.height=3.5, message=FALSE, warning=FALSE}
epa_data %>%
  filter(Year==2019) %>%
  ggplot(
    aes(
...
## Git Repository
https://github.com/carapk/EDE_Fall2023
