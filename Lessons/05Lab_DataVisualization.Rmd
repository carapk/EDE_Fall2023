...
# only need to specify expression becasue R^2 is a "formula"- if it's not you can just say "paste" for normal text
#Print the plot
print(Plot_text)
...
# two ways to do this- looks at lakes and see the type in environment or run class function
class(PeterPaul.chem.nutrients$month)

#List unique values
unique(PeterPaul.chem.nutrients$month)
#should have 1-12 but there are some months with no observations, 
#so these are missing- will have some months with no data when plotted
# must decide if you want to show all the months or just the ones you have data for 

#Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=month))+
         geom_bar()
# see that you don't have observations for all the months, and month is an integer column, not a factor
# if you wanted to color code by month, and use color as an aes, and we call month, ggplot won't know how to handel it 
# want to instead convert month to a facotr 

#Convert month to a factor -- with 12 levels, labelled with month names
# can call factor function or use asFactor
factor(PeterPaul.chem.nutrients$month,levels=1:12, labels=month.abb)
#labels: r has pre-defined labels and one set includes months of the year - month.abb = months of yr in abbr. format
# need to define levels, 1:12 means 1-12
...
#drop=FALSE will include months that do not have data so we have the missing months now
#name of scale gives it a better / more descriptive name
ggplot(PeterPaul.chem.nutrients,
       aes(x=factor(
         PeterPaul.chem.nutrients$month,
         levels=1:12,
...
#Tidy up the code
# spacing each command on its own line so things do not run past the edge
# can also change theme- themes control background color, size font and position of plot title,
# size plot and decision of x labels, ticks and gridlines, legend
...
# 1. 
# Plot surface temperatures by day of  year. - temp is y, date is x
...
ggplot(,
  ...
  # make points transparent by setting alpha to which % transparent you want
  scale_color_viridis(name="Latitutde", option="viridis", direction=-1)+
