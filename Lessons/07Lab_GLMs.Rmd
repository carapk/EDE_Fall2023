...
ggplot(EPAair,aes(x=Ozone,
  ...
  #need transparency to see the blue plot and the green because otherwise you wouldn't see both
  scale_fill_manual(values=c("darkgreen","darkblue"))+
  labs(x="Ozone AQI Value",
       y="Density",
       #This makes no legend title for the fill becuase it is not needed
       fill="")
...
ggplot(EPAair, 
  ...
  geom_boxplot()+
  # adding a new layer to the plot representing max value for each year
  stat_summary(geom="text",
               # could have said min, marks by adding text and label 
               fun=max,
               # max will be calculated for each factor in year column 
               vjust=-1,
               #size of text
               size=4,
               #this is the label
...
ggplot(EPAair, aes(x=as.factor(Year),
                   y=Ozone))+
  #need the 0.5 so we get the lines in the middle
...
PM2.5byOzone.regression <- lm(
  #specify dataframe
  data= EPAair,
  #two things you are comparing
  PM2.5 ~ Ozone
)
#run summary to find r squared and p value
...
# if you have more than one variable, always look at adjusted r-squared
# if you have one variable, look at multiple r squared
